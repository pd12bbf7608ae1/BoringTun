...
# 		! command -v "${WG_QUICK_USERSPACE_IMPLEMENTATION:-wireguard-go}" >/dev/null && exit $ret
...
del_if() {
	...
	# cmd ip link delete dev "$INTERFACE" # kill boringtun
}
...
cmd_up() {
	local i
	# [[ -z $(ip link show dev "$INTERFACE" 2>/dev/null) ]] || die "\`$INTERFACE' already exists" # disable check
	# trap 'del_if; exit' INT TERM EXIT
	execute_hooks "${PRE_UP[@]}"
	# add_if # using boringtun
	set_config
